...
namespace Basic
{
    class Convert
    {
        static void Main(string[] args)
        {
            ...
            int baseNum = 2;
            string s = System.Convert.ToString(value, baseNum); // value를 string으로 변환(2진수로)
            int i = System.Convert.ToInt32(s, baseNum); // string으로 변환된 값을 2진수로 읽어서 int로 다시 변환
            Console.WriteLine("i = {0}, {1,2}진수 = {2,16}", i, baseNum, s);

            baseNum = 8;
            s = System.Convert.ToString(value, baseNum);
            i = System.Convert.ToInt32(s, baseNum);
            Console.WriteLine("i = {0}, {1,2}진수 = {2,16}", i, baseNum, s);
